...
\begin{table}[ht!]
    ...
    \begin{tabular}{|c|c|c|c|}
        \hline
        \textbf{Sampler} & \textbf{Average Throughput (M/ms)} & \textbf{Standard Deviation} \\
        \hline
        FIO & 26.2 & 0.01 \\
        \hline
...
From the table, we can recognize that the Sysbench sampler is the fastest, which is to be expected given the overhead of the JVM. But to further investigate why this difference is present, a
run with a profiler is required. Running \verb|perf|\cite{PerfLinuxProfilinga} along with the JMH tool yields the following results for YCSB and ApacheCommons (only relevant lines):

\begin{perfbox}{ApacheCommons}
\ttfamily
